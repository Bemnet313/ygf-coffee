...
export const translations: Record<string, { en: string; am: string }> = {
    ...
    ABOUT_P1: { en: 'YGF is a family-run Ethiopian coffee export business, built on three generations of knowledge and care. The name comes from our roots — Y for Yishak, Yacob, and Yoseph, G for their father Girum, and F for their mother Faith. Together, the Baleme family brings Ethiopian specialty coffee to the world from our base in Addis Ababa.', am: 'ዋይጂኤፍ በሶስት ትውልድ ዕውቀትና ጥንቃቄ ላይ የተመሰረተ የኢትዮጵያ ቡና ኤክስፖርት ቤተሰባዊ ድርጅት ነው። ስሙ ከቤተሰብ ስሞች የተገኘ ነው — Y ለይስሃቅ፣ ለያዕቆብ እና ለዮሴፍ፣ G ለአባታቸው ግሩም፣ F ለእናታቸው ፌይዝ። በአንድነት የባለሜ ቤተሰብ ከአዲስ አበባ ለዓለም ምርጥ ቡና ያቀርባል።' },
    ABOUT_P2: { en: 'We source Grade 1 through 5 green coffee from Ethiopia\'s most celebrated regions — Yirgacheffe, Sidamo, Guji, Jimma, Limu, and Harrar. Every lot is hand-selected, carefully processed, and traceable from farm to port. We are here to build lasting partnerships with roasters and importers who value quality, transparency, and a direct line to origin.', am: 'ከይርጋጨፌ፣ ሲዳሞ፣ ጉጂ፣ ጅማ፣ ሊሙ እና ሐረር — ከታወቁ የቡና ክልሎች ከ1 እስከ 5 ያሉ ደረጃዎች አረንጓዴ ቡና እናቀርባለን። እያንዳንዱ ሎት በእጅ የተመረጠ፣ በጥንቃቄ የተዘጋጀ እና ከእርሻ እስከ ወደብ ምንጩ የሚታወቅ ነው። ጥራትን፣ ግልጽነትን እና ቀጥታ ግንኙነትን ከሚፈልጉ ሮስተሮች እና ኢምፖርተሮች ጋር ዘላቂ ትብብር ለመፍጠር እዚህ ነን።' },
